...
Documentation: [English](https://github.com/orinsid/OneDriveGUI/blob/main/README.md) | 简体中文
* * *
在 **arm64** 编译的Docker容器,带有OneDrive的GUI界面。**amd64为docker的多版本构建镜像，未进行测试。**

[Docker](https://hub.docker.com/repository/docker/orinsid/onedrivegui/general)项目地址

本项目是基于以下项目实现的，对他们表示诚挚的感谢：
...
OneDrive for Linux的配置请查看[使用文档](https://github.com/abraunegg/onedrive/blob/master/docs/USAGE.md)

**从头构建：** 请查阅分支[docker](https://github.com/orinsid/OneDriveGUI/tree/docker)，将roofts文件夹和Dockerfile放置到同一目录，并使用Docker部署。
### 运行示例：
...
networks: {}
```
浏览`http://your-host-ip:5800`以访问 OneDriveGUI
...
例如，假设反向代理服务器与该容器运行在同一台机器上。服务器会将发送到 `onedrivegui.domain.tld` 的所有 HTTP 请求代理到位于 `127.0.0.1:5800` 的容器。
...
例如，假设反向代理服务器与该容器在同一台计算机上运行。 服务器会将“server.domain.tld/onedriveguir”的所有 HTTP 请求代理到“127.0.0.1:5800”的容器。
